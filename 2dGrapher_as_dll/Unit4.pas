...
//рассчитали значение операции...
          ...
          if IsMathEx then
              result.result:=FDataAr[k-1].Data; //результат = Значение в последней ячейке данных
...
begin
   ...
   aType:=D; bType:=D; //Данные по умолчанию. Они НЕ ДОЛЖНЫ использоваться. Но
   ...
   1..5,9: begin //     +-*/^log : 2 args
           ...
           //-------------$$$$$$$$$$$$$$$$$$$$$$$$$$$$
           if not ADependonX then  //Если слева константа, то дополняем DataAr числом 0 - результатом дифференцирования
              begin
           ...
           if not BDependonX then  //Если справа константа, то дополняем DataAr числом 0 - результатом дифференцирования
              begin
              SetLength(result.DataAr,length(result.DataAr)+1);
...
function RestoreString(var Formula: TFormula; RestoringOper: integer): string; overload;
var i,j,k: integer;
abranches,bbranches: boolean; //окружать ли скобками левый и правый аргументы
