...
function M_ProgSLAU(const A: TMatrix; const B: TVect): TVect;
 ...
 (*    procedure ToLog(flag: integer);
     ...
     1: begin
        {-вывод KMax и заданной точности}
        writeln(log,
             '<tr><td colspan=5 height=30 align=left>требуемая точность: '+
             FloatToStr(XN_Epsilon)+'</td></tr>'+#13+'</table>');
        end;
     2: begin
             {-вывод KMax и заданной точности}
        writeln(log,
     ...
     100: begin{вывод решения Гаусса}
          DImAlpha:=length(Alpha[0]);
     writeln(log,
             '</tr></table>'+#13);
     {вывод решения. таблица, 2 строки:
     1я с пояснением, вторая - 2 ячейки:
     ...
     2) решение.}
     Writeln(log,
             '<table><tr><td colspan=2>решение:</td></tr>'
             +#13+'<tr>'+#13);
     {-1------------------------------------------}
     writeln(log,
             '<td align=center valign=center>'+
     ...
     {--------------------------------------------}
     lnout:='';
     if Not IsZero(Alpha[cI,0]) then lnout:=FloatToStrF(Alpha[cI,0],ffGeneral,7,5);

     for cJ := 1 to DimAlpha- 1 do
        begin
        if Not IsZero(Alpha[cI,cJ]) then
           if alpha[cI,cJ]>0 then
               if Not IsZero(Alpha[cI,cJ]-1)then
                   if not (lnout='') then
                      lnout:=lnout+'+'+FloatToStrF(Alpha[cI,cJ],ffGeneral,7,5)+'*c'+inttostr(cJ)
                   else
                      lnout:=FloatToStrF(Alpha[cI,cJ],ffGeneral,7,5)+'*c'+inttostr(cJ)
               else
                   if not (lnout='') then
                      lnout:=lnout+'+c'+inttostr(cJ)
                   else
                      lnout:='c'+inttostr(cJ)
           else
               if Not IsZero(Alpha[cI,cJ]+1)then
