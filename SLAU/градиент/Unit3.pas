...
begin
  ...
  1 - вывести начальное приближение,первоначальный вектор невязки, H0.
  ...
  0: begin
     ...
     //строка2 с данными: в ней - 3 ячейки.
     //X=[]; r=[]; H=_
     //1     2     3
     AssignFile(Outpt,'output.html');
     Append(Outpt);
     {вывод начального приближения. таблица, 2 строки:
     1я с пояснением(3 ячейки), вторая - 3 ячейки:
     1) X= собстна, значение первого приближения.
     2) r= и значение невязки
     3) H= начальное значение функционала}
     Writeln(Outpt,
             '<table><tr><td>Начальное приближение:</td><td>Вектор невязки начальный:'
             +'</td><td>начальное значение функционала H:</td></tr>'
             +#13+'<tr>'+#13);
     {-1------------------------------------------}
     writeln(Outpt,
 ...
 writeln(Outpt,'</td>');
  ...
  10:begin
     ...
     if XN<=XN_Epsilon then
         ...
         Writeln(Outpt,
             '<hr size=8 color=000000>'+
             '<table><tr><td colspan=2><font color=FFFFFF>итерационный процесс кончился,'+
